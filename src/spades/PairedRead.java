			...
			while (resulSet.next()) {
				...
				case "rf":
					String assemblyCmmd = "python3 " + "/opt/SPAdes/bin/spades.py " + spades_options + " "
							+ forwardReads + " " + output + "/2_treated.fastq" + " " + reverseReads + " " + output
							+ "/1_treated.fastq" + " -k " + spades_kmers + " -m " + spades_memory + " -t "
							+ spades_threads + " -o " + output + "spades-assembly";
				...
				statmt = DatabaseConnection.connect.prepareStatement(
						"UPDATE project SET status =  'Complete SPAdes' WHERE project.idproject=" + idproject + ";");
	...
	public void checkFileSpades(int idproject) {
		try {
			...
			if (a.exists()) {
				...
				ProjectLog.getLblYouCanCheck().setText("Something went wrong with SPAdes.");
				ProjectLog.getEmptyField().setText("Please, check the SPAdes log file!");
