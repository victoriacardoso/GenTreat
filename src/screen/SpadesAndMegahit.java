...
public class SpadesAndMegahit {
	...
	public void initialize() {
		...
		spinner_threads.setModel(new SpinnerNumberModel(16, 1, null, 1));
		...
		txtKmers.addKeyListener(new KeyAdapter() {
			@Override
			public void keyTyped(KeyEvent e) {
				String caracteres = "0987654321auto,";
		...
		lblGb.setBounds(131, 124, 70, 20);
		panel_inputParameters.add(lblGb);
		...
		dialogEven = new JDialog(frm_SpadesAndMegahit);
		dialogEven.setBounds(450, 300, 400, 150);
		dialogEven.setResizable(false);
		dialogEven.getContentPane().setLayout(null);
		...
		JLabel evenField = new JLabel();
		evenField.setBounds(15, 24, 385, 30);
		dialogEven.getContentPane().add(evenField);
		...
		JButton btnOkEvenError = new JButton("OK");
		btnOkEvenError.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				dialogEven.dispose();
			}
		});
		btnOkEvenError.setBounds(154, 66, 66, 19);
		dialogEven.getContentPane().add(btnOkEvenError);
			...
			public void actionPerformed(ActionEvent arg0) {
				try {
					...
					if (!spadesKmer.isBlank() && !spadesMemory.isBlank() && !spadesThreads.isBlank()
						...
						boolean kmersOKSPAdes = true;

						if (!spadesKmer.equals("auto")) {
							if (!spadesKmer.matches("[\\,0-9]*")) {
								kmersOKSPAdes = false;
							}
							else if (spadesKmer.matches("[\\,0-9]*")) {
								String[] kmers = spadesKmer.split(",");
								for (int i = 0; i < kmers.length; i++) {
									if (Integer.parseInt(kmers[i]) % 2 == 0 || Integer.parseInt(kmers[i]) > 128) {
										kmersOKSPAdes = false;
										break;
									}
								}
						...
						boolean kmersOKMegahit = true;
						String[] kmers = megahit_kmers.split(",");

						for (int i = 0; i < kmers.length; i++) {
							if (Integer.parseInt(kmers[i]) % 2 == 0 || Integer.parseInt(kmers[i]) < 15
									|| Integer.parseInt(kmers[i]) > 255) {
								kmersOKMegahit = false;
								break;
							}

						}
						if (kmersOKSPAdes && kmersOKMegahit) {
							int idproject = 0;
							Statement st = null;
							st = DatabaseConnection.connect.createStatement();
							idproject = st.executeQuery("select last_insert_rowid()").getInt(1);

							stt = DatabaseConnection.connect.prepareStatement("UPDATE parameter SET mem_flag='"
									+ mem_flag + "'," + "min_count='" + min_count + "'," + "megahit_kmers='"
									+ megahit_kmers + "' WHERE idproject=" + idproject);
							stt.executeUpdate();

							if (getComboBox_library().getSelectedItem().toString().equals("Single-read")) {

								stmt = DatabaseConnection.connect
										.prepareStatement("UPDATE parameter SET spades_options='" + getSelectedOption()
												+ "'," + "spades_kmers='" + spadesKmer + "'," + "spades_memory='"
												+ spadesMemory + "'," + "spades_threads='" + spadesThreads + "',"
												+ "orientation=" + null + " WHERE idproject=" + idproject);
								stmt.executeUpdate();
								...
								stmt = DatabaseConnection.connect
										.prepareStatement("UPDATE parameter SET spades_options='" + getSelectedOption()
												+ "'," + "spades_kmers='" + spadesKmer + "'," + "spades_memory='"
												+ spadesMemory + "'," + "orientation='"
												+ getComboBox_Orientation().getSelectedItem().toString() + "',"
												+ "spades_threads='" + spadesThreads + "' WHERE idproject="
												+ idproject);
								stmt.executeUpdate();
							...
							if (kmersOKSPAdes == false) {
								dialogEven.setTitle("SPAdes Error");
								evenField.setText("List of kmers sizes must be odd and less than 128!");
							} else if (kmersOKMegahit == false) {
								dialogEven.setTitle("Megahit Error");
								evenField.setText("List of kmers sizes must be odd and between 15-255!");

							}
							dialogEven.setVisible(true);
		...
		txt_kmerSizeMegahit.addKeyListener(new KeyAdapter() {
			@Override
			public void keyTyped(KeyEvent e) {
